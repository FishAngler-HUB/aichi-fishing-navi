...
rem Fetches the latest fishing report from Google Drive (via rclone)
rem and rebuilds data\catches.json. Does NOT touch git.
...
>>"%LOG%" echo.
>>"%LOG%" echo ===== %date% %time% run-update-catches.bat start =====
...
>>"%LOG%" echo node=%NODE%
>>"%LOG%" 2>&1 "%NODE%" "tools\update-catches.js"
set "EC=%ERRORLEVEL%"
